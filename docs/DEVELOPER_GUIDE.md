...
>
> **موازنة نقدية (`cash_budget`, 2026-08-20):** بعد البنوك في الشريط. لقطة أرصدة حتى نهاية الفترة — الالتزامات في جدول واحد (اسم الحساب الفرعي فقط + المشروع؛ بدون عنوان «التزامات (مستحق علينا)»). الإجماليات أعلى التقرير = بنوك `12101` + خزينة `12102` + مستخلصات `12201`. **بعد الاعتماد:** حذف المستبعد · توزيع **بنوك `12101` فقط** (كل أوراق `12102` عهد/صناديق — لا تدخل مجمع السداد) وبحد الالتزامات · عمود نسبة التوزيع · جدول إجمالي لكل مشروع (شاشة + طباعة). **لا قيد GL.**
