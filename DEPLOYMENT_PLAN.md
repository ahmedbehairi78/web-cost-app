...
**آخر تحديث:** 2026-08-13 — **قفل خمول Electron:** نشاط الجهاز كله يُبقي التطبيق؛ بعد 3 دقائق شاشة دخول + اسم المستخدم **مع كلمة المرور** (لا إغلاق). **شجرة الأصناف Excel:** قالب/استيراد مطابق ملف المخزن v2 (عمود `Code` = `name_en` · الرصيد يُتجاهل · تحديث الأكواد الموجودة) · **الوضع الافتراضي:** بعد النجاح رسالة + زر إعادة الدخول (`app.relaunch` / reload) بدل الإغلاق المفاجئ · شركة فارغة + COA، إبقاء `myline78@gmail.com` · نسخة Postgres كاملة (`prod:export-backup` / `local:export-backup`) قبل رفع بيانات الشركة · إصلاح تصدير `warehouse_receipts`. 2026-08-08 — تفكيك ActualCosts/Inventory · تخفيف lag Electron.
