...
| `server/src/migration/buildPostgresBackup.ts` | Full Postgres snapshot — **73** collection keys (inventory · payroll · fixed assets · OHA · custody · MOS · VO · …); v3 JSON via **`GET /api/settings/backup-export`** |
...
|-----|-------------|-------------|
| فاتورة مشتريات | `invoice` | Purchase invoice — **آجلة** (Cr **21101…** مورد) أو **نقدية** (Cr **12102…** عهدة/صندوق) عبر `paymentType` · **`recordPurchaseToProjectInventory()`** (Dr **127…** warehouse incl. VAT, Cr الطرف المختار + WHT) **or** toggle **«تسجيل كأصل ثابت»** → **`recordFixedAssetPurchase()`** (Dr **11…** asset incl. VAT); **`SearchableSelect`** for **11…** COA leaves + **amount (ex-VAT)** field (no material lines / warehouse). **Local:** `invoiceLines` persisted on `purchase_transaction_items` via API. **Lines (2026-07-24):** multi-material lines; each line may link **multiple BOQ items** (`boqItemIds[]` + legacy `boqItemId`); BOQ picker filtered by selected **cost center (contract)** else by **warehouse project**; description/unit/qty/cost = **purchased material** only (never overwrite from BOQ rates). زر **«فاتورة جديدة»** أعلى الشريط الجانبي. |
...
4. **Consumption** — `POST /api/consumption-orders` with optional **`lines[]`** (multi-BOQ allocation per material) + **`expenseAccountCode` / `expenseAccountName`** on header; then `POST /api/consumption-orders/:id/confirm`: atomic **`prisma.$transaction`** — project inventory issue, **`boq_actual_costs`** per line, GL via **`consumptionJournal.ts`** (multiple Dr expense + one Cr **127…**; `costCenterId = contractId`). UI: **`ConsumptionOrderModal`** + **`ConsumptionAllocationModal`**; templates via **`/api/consumption-allocation-templates`**. GL reference = `order_number` (already `CON-…`). List/filter API scopes by **`project_id IN (accessible projects)`**. **Do not use** legacy `POST /api/inventory/consume` or client **`recordConsumptionIssue`** on confirm.
...
8. **Available qty** = `quantity_balance` on `project_inventory` (reserved qty already deducted in generated balance).
