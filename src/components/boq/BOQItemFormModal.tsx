...
interface Props {
  ...
  /** default = نموذج كامل؛ changeOrder = أوامر التغيير (الهوية أساسية للقراءة فقط) */
  variant?: 'default' | 'changeOrder';
  /** Seeded when the modal opens (edit / change-order / new). */
  initialData: BoqItemFormData;
  /** Contract id for offline draft key (new items only). */
  contractId?: string;
  isSubmitting: boolean;
  onSubmit: (resolved: BoqItemFormData) => void | Promise<void>;
  onClose: () => void;
  theme: string;
  language: string;
...
export function BOQItemFormModal({
  ...
  const [formData, setFormData] = useState<BoqItemFormData>(initialData);
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof BoqItemFormData, string>>>({});

  const offlineUserId = useOfflineUserId();
  const draftEnabled = Boolean(
    isLocalBackend && isOpen && !editingItem && !isChangeOrder && contractId,
  );
  const {
    clearDraft,
    restorePrompt,
    ...
    draftKey: contractId ? FORM_DRAFT_KEYS.boqItemNew(contractId) : 'boq_item:none',
    value: formData,
    enabled: draftEnabled,
    isEmpty: (v) => !String(v.itemCode || '').trim() && !String(v.description || '').trim(),
  });

  // Seed once per open / edit target — do not depend on `initialData` identity (avoids reset while typing).
  useEffect(() => {
    if (!isOpen) return;
    ...
    setNewSectionName('');
    setErrors({});
    // eslint-disable-next-line react-hooks/exhaustive-deps -- seed from props only when modal opens or edit target changes
  }, [isOpen, editingItem?.id, variant]);
  ...
  const uniqueChapters = useMemo(() => {
    ...
    existingItems.forEach((i) => {
      if (i.chapterCode) map.set(i.chapterCode, i.chapterName || i.chapterCode);
    });
  ...
  const uniqueWorkTypes = useMemo(() => {
    ...
    existingItems
      .filter((i) => i.chapterCode === chCode)
      .forEach((i) => {
        if (i.workTypeCode) set.add(i.workTypeCode);
      });
  ...
  const uniqueSections = useMemo(() => {
    ...
    existingItems
      ...
      .forEach((i) => {
        if (i.sectionCode) map.set(i.sectionCode, i.sectionName || i.sectionCode);
      });
    return Array.from(map.entries())
      .map(([code, name]) => ({ code, name }))
      .sort((a, b) => a.code.localeCompare(b.code, undefined, { numeric: true }));
  ...
  useEffect(() => {
    if (!editingItem && !isChangeOrder && suggestedItemCode && !formData.itemCode) {
      setFormData((prev) => ({ ...prev, itemCode: suggestedItemCode }));
    }
  }, [suggestedItemCode]); // eslint-disable-line react-hooks/exhaustive-deps

  const set = (key: keyof BoqItemFormData, value: string | number) => {
  ...
  const handleChapterChange = (code: string) => {
    ...
    setFormData((prev) => ({
      ...prev,
      chapterCode: code,
      chapterName: code === NEW_OPTION ? '' : name,
      workTypeCode: '',
      sectionCode: '',
      sectionName: '',
      itemCode: '',
  ...
  const handleSectionChange = (code: string) => {
    ...
    setFormData((prev) => ({
      ...prev,
      sectionCode: code,
      sectionName: code === NEW_OPTION ? '' : name,
      itemCode: '',
    }));
    setNewSectionCode('');
    setNewSectionName('');
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    const resolved: BoqItemFormData = isChangeOrder
      ? { ...formData }
      : {
          ...formData,
    ...
    try {
      ...
      if (!editingItem && !isChangeOrder) await clearDraft();
    } catch {
      /* parent handles errors / queued offline */
    }
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          ...
          >
            ...
            <form onSubmit={(e) => void handleFormSubmit(e)} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
              {!editingItem && !isChangeOrder && (
                <FormDraftRestoreBanner
                  show={Boolean(restorePrompt)}
                  updatedAt={restorePrompt?.updatedAt}
                  onRestore={() => {
                    const p = restorePrompt?.payload;
                    if (p) setFormData(p);
                    acceptRestore();
                  }}
                  onDiscard={() => {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        {uniqueChapters.map((c, ci) => (
                          <option key={c.code || `chapter-${ci}`} value={c.code}>
                            {c.code} — {c.name}
                          </option>
                        ))}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        {uniqueSections.map((s, si) => (
                          <option key={s.code || `section-${si}`} value={s.code}>
                            {s.code} — {s.name}
                          </option>
                        ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {isChangeOrder ? (
                  <div
                    className={cn(
                      'p-4 rounded-xl border space-y-3 h-fit',
                      theme === 'dark'
                  ...
                  >
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wide">
                      {isAr ? 'البيانات الأساسية (قراءة فقط)' : 'Core item data (read-only)'}
                    </p>
                    <div className="space-y-2 text-xs">
                      <div>
                        <span className="text-gray-400 block mb-0.5">{isAr ? 'الفصل' : 'Chapter'}</span>
                        <span className="font-mono font-medium">
                          {formData.chapterCode || '—'}
                          {formData.chapterName ? ` — ${formData.chapterName}` : ''}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-400 block mb-0.5">{isAr ? 'نوع العمل' : 'Work type'}</span>
                        <span className="font-mono font-medium">{formData.workTypeCode || '—'}</span>
                      </div>
                      <div>
                        <span className="text-gray-400 block mb-0.5">{isAr ? 'القسم' : 'Section'}</span>
                        <span className="font-mono font-medium">
                          {formData.sectionCode || '—'}
                          {formData.sectionName ? ` — ${formData.sectionName}` : ''}
                        </span>
                      </div>
                      <div className="grid grid-cols-2 gap-2 pt-1 border-t border-gray-700/20 dark:border-gray-600/30">
                        <div>
                          <span className="text-gray-400 block mb-0.5">{isAr ? 'كود البند' : 'Item code'}</span>
                          <span className="font-mono font-bold text-blue-400">{formData.itemCode || '—'}</span>
                        </div>
                        <div>
                          <span className="text-gray-400 block mb-0.5">{isAr ? 'الوحدة' : 'Unit'}</span>
                          <span className="font-medium">{formData.unit || '—'}</span>
                        </div>
                      </div>
                      <div>
                        <span className="text-gray-400 block mb-0.5">{isAr ? 'الوصف' : 'Description'}</span>
                        <p className="text-sm leading-snug whitespace-pre-wrap">{formData.description || '—'}</p>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <datalist id="unit-suggestions">
                          {['م3', 'م2', 'م ط', 'طن', 'عدد', 'كجم', 'لتر', 'م'].map((u) => (
                    ...
                    <div className="space-y-1">
                      <label className={labelCls}>
                        {isAr ? 'وصف البند' : 'Item Description'}
                        <span className="text-red-500 ml-1">*</span>
                      </label>
                      <textarea
                        rows={3}
                ...
                >
                  <div className="flex justify-between items-center border-b border-gray-800 pb-2 mb-1">
                    <span className="text-xs font-bold text-gray-500 uppercase">
                      {isAr ? 'ملخص الحساب' : 'Calculation'}
                    </span>
                    <Calculator size={16} className="text-purple-400" />
                  </div>
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-gray-400">{isAr ? 'تكلفة مباشرة' : 'Direct Cost'}</span>
                      <span className="font-mono">{formatNumber(direct)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">
                        {isAr ? 'مصاريف' : 'Overhead'} ({formData.rateOverheadPct}%)
                      </span>
                      <span className="font-mono">{formatNumber(overheadAmt)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">
                        {isAr ? 'ربح' : 'Profit'} ({formData.rateProfitPct}%)
                      </span>
                      <span className="font-mono text-purple-400">{formatNumber(profitAmt)}</span>
                    </div>
                    <div className="pt-2 border-t border-gray-800 flex justify-between items-center">
                      <span className="font-bold text-blue-400">{isAr ? 'سعر الوحدة' : 'Unit Rate'}</span>
                      <span className="font-mono font-black text-blue-400">{formatNumber(unitRate)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-green-400">{isAr ? 'إجمالي البند' : 'Item Total'}</span>
                      <span className="font-mono font-black text-green-400">{formatNumber(tenderAmount)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                {(
                  ...
                  ] as [keyof BoqItemFormData, string][]
                ).map(([key, label]) => (
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="0"
                    className={cn(errors.tenderQty ? errCls : cn(inputCls, 'font-mono'))}
                    value={formData.tenderQty || ''}
                    onChange={(e) => set('tenderQty', Number(e.target.value))}
                  />
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="number"
                    step="0.1"
                    title={isAr ? 'المصاريف %' : 'Overhead %'}
                    placeholder="0"
                    className={cn(inputCls, 'font-mono')}
                    value={formData.rateOverheadPct || ''}
                    onChange={(e) => set('rateOverheadPct', Number(e.target.value))}
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <input
                    type="number"
                    step="0.1"
                    title={isAr ? 'الربح %' : 'Profit %'}
                    placeholder="0"
                    className={cn(inputCls, 'font-mono')}
                    value={formData.rateProfitPct || ''}
                    onChange={(e) => set('rateProfitPct', Number(e.target.value))}
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    type="date"
                    title={isAr ? 'تاريخ بدء العمل' : 'Start Date'}
                    className={inputCls}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="number"
                    title={isAr ? 'مدة التنفيذ بالأيام' : 'Duration in Days'}
                    placeholder="0"
                    className={cn(inputCls, 'font-mono')}
                    value={formData.expectedDuration || ''}
                    onChange={(e) => set('expectedDuration', Number(e.target.value))}
                  />
                </div>
              </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    theme === 'dark'
                      ...
                      : theme === 'soft'
                        ? 'bg-[#cfd8dc] hover:bg-[#b0bec5] text-[#37474f]'
