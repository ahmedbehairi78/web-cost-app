...
| التحديث | تنبيه في الجرس بعد نشر Railway → المستخدم يبدأ التحديث → شاشة الدخول (لا إغلاق مفاجئ) | كاش Electron + chunk 404 كانا يعيدان التحميل فجأة |
...
- **2026-08-20:** موديول **بدجيت الالتزامات** (`cash_budget`) — تخطيط فقط (لا GL) · migration `20260820120000_cash_budget` + `chart_of_accounts.min_balance`. يلزم `migrate deploy` على Railway.
...
**آخر تحديث:** 2026-08-13 — **أرصدة افتتاحية:** لكل مشروع من رصيد المخزن (قالب كود/كمية/تكلفة · ربط 127 على الشاشة الفارغة · رفض ملف شجرة الأصناف). **قفل خمول Electron:** نشاط الجهاز كله يُبقي التطبيق؛ بعد 3 دقائق شاشة دخول + اسم المستخدم **مع كلمة المرور** (لا إغلاق). **شجرة الأصناف Excel:** قالب/استيراد مطابق ملف المخزن v2 (عمود `Code` = `name_en` · الرصيد يُتجاهل · تحديث الأكواد الموجودة) · **الوضع الافتراضي:** بعد النجاح رسالة + زر إعادة الدخول (`app.relaunch` / reload) بدل الإغلاق المفاجئ · شركة فارغة + COA، إبقاء `myline78@gmail.com` · نسخة Postgres كاملة (`prod:export-backup` / `local:export-backup`) قبل رفع بيانات الشركة · إصلاح تصدير `warehouse_receipts`. 2026-08-08 — تفكيك ActualCosts/Inventory · تخفيف lag Electron.
