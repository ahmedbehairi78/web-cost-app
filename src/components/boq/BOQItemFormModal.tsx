...
export function BOQItemFormModal({
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!validate()) return;
    const resolved: FormData = isChangeOrder
      ? { ...formData }
      : {
          ...formData,
          ...
          >
            <div className={cn('p-6 border-b flex justify-between items-center', theme === 'dark' ? 'bg-gray-900/50 border-gray-800' : theme === 'soft' ? 'bg-[#eceff1] border-[#cfd8dc]' : 'bg-gray-50 border-gray-200')}>
              <h3 className="text-xl font-bold">
                {isChangeOrder
                  ? (isAr ? 'أوامر التغيير' : 'Change orders')
                  : editingItem
                    ? (isAr ? 'تعديل البند' : 'Edit Item')
                    : (isAr ? 'إضافة بند جديد' : 'Add New Item')}
              </h3>
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {uniqueChapters.map((c, ci) => (
                      <option key={c.code || `chapter-${ci}`} value={c.code}>{c.code} — {c.name}</option>
                    ))}
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {uniqueSections.map((s, si) => (
                      <option key={s.code || `section-${si}`} value={s.code}>{s.code} — {s.name}</option>
                    ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {isChangeOrder ? (
                <div className={cn('p-4 rounded-xl border space-y-3 h-fit', theme === 'dark' ? 'bg-gray-900/40 border-gray-800/80' : theme === 'soft' ? 'bg-[#eceff1] border-[#cfd8dc]' : 'bg-gray-50 border-gray-200')}>
                  ...
                  <div className="space-y-2 text-xs">
                    <div>
                      ...
                      <span className="font-mono font-medium">{formData.chapterCode || '—'}{formData.chapterName ? ` — ${formData.chapterName}` : ''}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="font-mono font-medium">{formData.sectionCode || '—'}{formData.sectionName ? ` — ${formData.sectionName}` : ''}</span>
                    </div>
                ...
                <div className={cn('p-4 rounded-xl border space-y-3 h-fit', theme === 'dark' ? 'bg-gray-900/30 border-gray-800/50' : theme === 'soft' ? 'bg-[#eceff1] border-[#cfd8dc]' : 'bg-gray-50 border-gray-100 shadow-sm')}>
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between"><span className="text-gray-400">{isAr ? 'تكلفة مباشرة' : 'Direct Cost'}</span><span className="font-mono">{formatNumber(direct)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">{isAr ? 'مصاريف' : 'Overhead'} ({formData.rateOverheadPct}%)</span><span className="font-mono">{formatNumber(overheadAmt)}</span></div>
                    <div className="flex justify-between"><span className="text-gray-400">{isAr ? 'ربح' : 'Profit'} ({formData.rateProfitPct}%)</span><span className="font-mono text-purple-400">{formatNumber(profitAmt)}</span></div>
                    <div className="pt-2 border-t border-gray-800 flex justify-between items-center"><span className="font-bold text-blue-400">{isAr ? 'سعر الوحدة' : 'Unit Rate'}</span><span className="font-mono font-black text-blue-400">{formatNumber(unitRate)}</span></div>
                    <div className="flex justify-between items-center"><span className="font-bold text-green-400">{isAr ? 'إجمالي البند' : 'Item Total'}</span><span className="font-mono font-black text-green-400">{formatNumber(tenderAmount)}</span></div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="number" step="0.01" title={isAr ? 'الكمية' : 'Quantity'} placeholder="0" className={cn(errors.tenderQty ? errCls : cn(inputCls, 'font-mono'))} value={formData.tenderQty || ''} onChange={e => set('tenderQty', Number(e.target.value))} />
              ...
              <div className="pt-4 flex gap-3">
                ...
                >
                  ...
                  {isChangeOrder
                    ? (isAr ? 'حفظ أوامر التغيير' : 'Save change orders')
                    : editingItem
                      ? (isAr ? 'تحديث البند' : 'Update Item')
                      : (isAr ? 'حفظ البند' : 'Save Item')}
                </button>
                <button
                  type="button"
                  onClick={onClose}
