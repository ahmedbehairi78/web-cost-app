...
**إدخال شبيه بإكسل (2026-08-08):** عند التركيز على حقل نص/رقم يُحدَّد المحتوى بالكامل (الكتابة تستبدل القيمة). داخل جداول قابلة للتحرير: أسهم ←→↑↓ + Enter + Tab للتنقل بين الخلايا. التثبيت من `main.tsx` عبر `installExcelLikeInputBehavior()` (`src/lib/excelLikeInputs.ts`). استثناء: `data-excel-nav="off"`.
