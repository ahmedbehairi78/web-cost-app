...
| **مستخلص خدمة** | نفس الدورة · `type=service_ipc` | رقم تسلسلي قصير (`1`، `2`، `6`…) يُنشأ على الخادم إن فُرغ المرجع؛ عنوان الطباعة `مستخلص — المورد الرقم`؛ اعتماد يرحّل مدين حسب `serviceKind` مع `costCenterId` لكل عقد؛ **لا** توزيع على بنود BOQ |
