...
- **Cash budget Excel:** من معاينة الطباعة (`تصدير Excel`) — شيت واحد عبر [`exportExcel.ts`](src/lib/reportDocument/exportExcel.ts) = نفس محتوى المعاينة (التزامات + إجمالي المشروع).
...
- **`src/lib/reportPrintProfiles.ts`** — single source for print layout. `ReportPrintProfile = { orientation, pageSize (A4/A3), density (compact/normal/comfortable), accent (hex), showHeader, showFooter, tableCellAlign (auto/start/center/end), bodyFontSize/bodyTextColor/tableShade/tableBorder/bodyBold/bodyItalic/bodyUnderline, … }`.
...
## 🔴 HANDOFF — إقفال متبقي يعمل بعد الافتتاحي + asOf اليوم ✅ (2026-09-01)

> **مشكلة:** الواجهة ظهرت (تواريخ/زر) لكن الإقفال لا يصفّر الفرق — غالباً الدورة `opening_posted` فكان الزر مخفياً/مرفوضاً، أو الأرصدة المفتوحة بعد `periodEnd` بينما الإقفال يحسب حتى نهاية الفترة فقط.

| المجال | ملخص |
|--------|------|
| **Residual** | يعمل من `pl_closed` / `bs_approved` / `opening_posted` — يلغي الافتتاحي + يفتح القفل ثم يرحّل `YE-PL-…-Rn` |
| **asOf** | إن صُفّر حتى `periodEnd` وما زال مفتوحاً حتى اليوم → إقفال حتى **اليوم** |
| **UI** | زر المتبقي يظهر طالما بقيت أرصدة `4/5`؛ تنبيه إلغاء الافتتاحي |

```powershell
npm run test -- server/src/accounting/fiscalPeriodClosing.test.ts
# أعد تشغيل/انشر API · فترات → إقفال المتبقي → اعتماد الميزانية · تقارير asOf مطابق
```

---

## 🔴 HANDOFF — إقفال متبقي + تواريخ القيود غير المقفلة ✅ (2026-09-01)

> **جلسة 2026-09-01:** بعد YE-PL بقي فرق ميزانية = أرصدة `4/5` مفتوحة (ترحيل لاحق أو ناقص). النظام كان يعتمد الميزانية بدون تصفير الدخل.

| المجال | ملخص |
|--------|------|
| **إقفال متبقي** | `POST /api/fiscal-closings/:id/close-income-residual` · `YE-PL-{label}-Rn` · يعيد اعتماد الميزانية إن لزم |
| **اعتماد BS** | مرفوض (409) طالما بقيت أرصدة `4…`/`5…` حتى `periodEnd` |
| **معاينة** | `openPlFirstDate` · `openPlLastDate` · عدد الحسابات المفتوحة في إعداد قائمة الدخل |
| **UI** | زر «إقفال المتبقي» + شريط تواريخ أول/آخر قيد تشغيلي غير مقفل |

```powershell
npm run test -- server/src/accounting/fiscalPeriodClosing.test.ts
# أعد تشغيل API → فترات → إعداد قائمة الدخل → إقفال المتبقي إن ظهر تنبيه · ثم اعتماد الميزانية
```

**لا تراجع:** لا تعتمد الميزانية مع `4/5` مفتوحة؛ لا تحذف مسار residual عند إعادة الفتح (soft-delete كل `YE-PL-{label}*`).

---

## 🔴 HANDOFF — ميزان/ميزانية: تاريخ «حتى» = نهاية الإقفال ✅ (2026-09-01)

> **متابعة:** المستخدم لم يرَ أثراً بعد التجميع الكامل — غالباً لأن الميزانية كانت **حتى اليوم** بينما نشاط `4`/`5` بعد `periodEnd` يعيد فتح الفرق، وقائمة الدخل تستبعد `YE-PL` عمداً.

| المجال | ملخص |
|--------|------|
| **asOf** | `GET …/trial-balance?asOf=` · `…/balance-sheet?asOf=` — افتراضي اليوم؛ زر «استخدم نهاية الإقفال» |
| **YE-PL** | يُدرج حتى مع فلتر مشروع/عقود محدودة |
| **لا تراجع** | local TB/BS لا تعود لمسار 5000 قيد عند فشل الخادم |

```powershell
npm run test -- server/src/lib/glReportBalances.test.ts
# انشر + Ctrl+Shift+R · تقارير → ميزانية → «حتى تاريخ» = نهاية الإقفال
```
...
## 🔴 HANDOFF — ترقيم مستخلص الخدمة عند الاعتماد فقط + تنقل Excel ✅ (2026-08-31)

> **جلسة 2026-08-31:** الترقيم وصل 9 رغم وجود معتمدين فقط — المسودات كانت تستهلك التسلسل. المطلوب: رقم عند الاعتماد فقط + أسهم/Tab في جدول البنود كإكسل.

| المجال | ملخص |
|--------|------|
| **ترقيم** | لا تعيين عند create/update/list؛ عند **approve** فقط من أقران `approved`؛ مسح رقم المسودة عند الحفظ |
| **Excel** | جدول مستخلص الخدمة ضمن التنقل العام (input + select) · أسهم لا تغيّر خيارات القائمة |
| **لا تراجع** | لا backfill أرقام عند `GET /purchase-transactions` |

```powershell
npm run test -- server/src/lib/serviceIpcNumber.test.ts src/lib/excelLikeInputs.test.ts
# أعد تشغيل API ثم مستخلص خدمة جديد → بلا رقم حتى الاعتماد → التالي = معتمد+1
```
...
## 🔴 HANDOFF — رقم وعنوان مستخلص الخدمة + الشعار ✅ (2026-08-30)

> **جلسة 2026-08-30:** الرقم كان UUID؛ العنوان «مستخلص خدمة — {uuid}»؛ شعار الشركة لا يظهر في معاينة Blob.
...
## 🔴 HANDOFF — ملخص مستخلص الخدمة أسفل البنود ✅ (2026-08-30)

> **جلسة 2026-08-30:** الملخص كان 3 أسطر فوق الجدول (قيمة الفترة + ضريبة + صافي). المطلوب: سابق · حالي · إجمالي · ضريبة · محتجز ضمان وتأمينات على **إجمالي الأعمال** · أي محتجز آخر · الصافي − مقدمة − مسدد سابق = المستحق، **أسفل** جدول البنود.

```powershell
npm run test -- src/lib/serviceContractor.test.ts src/lib/reportDocument/reportDocument.test.ts src/lib/operationsManual.test.ts
```
...
| المجال | ملخص |
|--------|------|
| **قشرة** | `electron/hostedSpaUpdate.ts` يستطلع `spa-build.json` من العملية الرئيسية · حوار أصلي **لاحقاً = الافتراضي** · الآن = مسح كاش + إعادة تحميل **مع بقاء الجلسة** |
| **SPA** | جرس: الآن/لاحقاً بعد تأكيد · **لا** توست ويندوز للتحديث (يمنع النقر العرضي) · `applyHostedSpaUpdate` لا يسجّل خروجاً |
| **401** | انتهاء الجلسة أثناء النشر **لا** يستدعي `app.quit` — شاشة الدخول داخل النافذة |
| **electron-updater** | `checkForUpdates()` بلا toast يعيد التشغيل · حوار التثبيت الافتراضي **لاحقاً** |

```powershell
npm run test -- src/lib/spaBuild.test.ts
npm run electron:build:shell
# المثبّت الحالي لن يرى حوار القشرة حتى electron:publish
# مؤقتاً: Ctrl+Shift+R يحمّل الواجهة الجديدة دون خروج
```
