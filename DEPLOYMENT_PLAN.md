...
| التحديث | تنبيه في الجرس بعد نشر Railway → **الآن أو لاحقاً** → شاشة الدخول فقط عند الاختيار (لا إغلاق مفاجئ) | كاش Electron + chunk 404 كانا يعيدان التحميل فجأة |

- **مصدر الصلاحيات (2026-08-13):** مربعات العرض/الإنشاء/التعديل في `users.permissions` — ليست حزم أدوار. `settings: true` لإدارة المستخدمين والنسخ الاحتياطي.
...
| نسخ احتياطي Postgres كامل | ✅ 2026-08-13 | `D:\cost web app\backups\20260813105359-production\` (+ local) — 78 مجموعة · مستخدمون + `passwordHash` · استلام مخزن |
...
**آخر تحديث:** 2026-09-05 — مستخلصات عميل/مقاول: قيمة الأعمال = كمية × فئة × نسبة إنجاز يدوية؛ بنود مقاول فرعية + `clientBoqItemId` + استيراد إكسل؛ الكفر دون تغيير معادلات الخصم. 2026-08-31 — المسدد يشمل تحويل بنك بلا مركز تكلفة (`projectIds` اختياري) + `fetchContractorCashPaidAmount` (API ثم احتياطي). `GET /api/gl/contractor-cash-payments`. إصلاح إقلاع: لا تستورد `costCenterAttribution` من `serviceContractor.ts`. 2026-08-30 — مستخلص مقاول (`ipc`) بنفس رقم/عنوان/شعار/ملخص مستخلص الخدمة (تسلسل منفصل لكل نوع). ملخص مستخلص الخدمة أسفل البنود (سابق/حالي/إجمالي + محتجزات على الإجمالي). محاذاة حقول رأس الشهادة من الشريط العائم (`slot: kv`). مستخلص الخدمة: رقم لكل مورد/سنة · عنوان = الرقم + الحالة · شعار واسم الشركة معاً. 2026-08-28 — مستخلص خدمة (`service_ipc`) + `suppliers.serviceKind` (عمال/معدات/سيارات/سكن تحت 21102، عدة مراكز تكلفة، فصل BOQ اختياري). 2026-08-21 — طباعة/PDF من معاينة التقرير: `sandbox` يشمل `allow-modals` (بدونها Chromium يمنع حوار الطباعة). شريط تنسيق عائم عند تحديد النص. إغلاق المعاينة لا يُسقط الجلسة (Blob iframe · Setup **1.0.9**). 2026-08-20 مساءً — جلسة Electron تبقى بعد نشر Railway (لا cold-start على reload · قفل بدل logout عند 401 · Setup **1.0.8**). 2026-08-20 مساءً — جلسة Electron تبقى بعد نشر Railway (لا cold-start على reload · قفل بدل logout عند 401 · Setup **1.0.8**). صباحاً: إشعار **الآن أو لاحقاً**. 2026-08-13 — **أرصدة افتتاحية:** لكل مشروع من رصيد المخزن (قالب كود/كمية/تكلفة · ربط 127 على الشاشة الفارغة · رفض ملف شجرة الأصناف). **قفل خمول Electron:** نشاط الجهاز كله يُبقي التطبيق؛ بعد 3 دقائق شاشة دخول + اسم المستخدم **مع كلمة المرور** (لا إغلاق). **شجرة الأصناف Excel:** قالب/استيراد مطابق ملف المخزن v2 (عمود `Code` = `name_en` · الرصيد يُتجاهل · تحديث الأكواد الموجودة) · **الوضع الافتراضي:** بعد النجاح رسالة + زر إعادة الدخول (`app.relaunch` / reload) بدل الإغلاق المفاجئ · شركة فارغة + COA، إبقاء `myline78@gmail.com` · نسخة Postgres كاملة (`prod:export-backup` / `local:export-backup`) قبل رفع بيانات الشركة · إصلاح تصدير `warehouse_receipts`. 2026-08-08 — تفكيك ActualCosts/Inventory · تخفيف lag Electron.
