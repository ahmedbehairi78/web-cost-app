...
| طباعة الشركة | `company_info` | **`PrintSettingsPanel`** (إعدادات العرض، admin): اسم/عنوان/ضريبة/شعار/نص تذييل فقط — **تصميم الصفحة من شريط التنسيق**؛ **تنسيق النص المحدد** من الشريط العائم يُحفظ مع **حفظ التصميم** (`selectionPatches`) ويُعاد تطبيقه عند فتح المعاينة |
