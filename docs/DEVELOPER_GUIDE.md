...
### 6.5.0c-2 موردون ومقاولو باطن + رصيد افتتاحي (2026-08-24)

| بند | تفصيل |
|-----|--------|
| UI | إعدادات → شجرة الحسابات و**التكاليف الفعلية** — `OpeningCreditorsImportPanel` (Postgres فقط) |
| Excel | النوع · الاسم · الرصيد الافتتاحي · كود اختياري — `creditorsOpeningExcel.ts` |
| API | `POST /api/suppliers/opening-import` |
| GL | قيد `AP-OPEN-…` — Dr جاري الشركاء `31401001` / Cr أوراق 21101… أو 21102… (سالب = مدين الجهة) |
| تكرار | يتخطّى الاسم+النوع المكرر ولا يكرر القيد إن وُجدت حركة على الورقة |
