...
- **إقفال قائمة الدخل + افتتاحي (2026-07-27):** `fiscal_period_closings` · إقفال `4/5 → 31301001` · **إقفال متبقي** `POST /api/fiscal-closings/:id/close-income-residual` إن بقيت أرصدة · معاينة تعرض `openPlFirstDate` / `openPlLastDate` · اعتماد ميزانية مرفوض مع `4/5` مفتوحة أو إن `|gap| > 1` جنيه (`BS_BALANCE_TOLERANCE`) · قيد `OPEN-…` (`journal_kind=fiscal_opening` مستبعد من تقارير الرصيد المستمر) · **بدون WIP**. واجهة: إعداد قائمة الدخل. اختبار: `npm run test -- server/src/accounting/fiscalPeriodClosing.test.ts`. لا تُعد معالجة أعمال تحت التنفيذ على `126…`.
