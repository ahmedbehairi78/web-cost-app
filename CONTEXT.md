...
**آخر تحديث:** 2026-08-30 (رقم مستخلص الخدمة تسلسلي قصير + عنوان الطباعة بالمورد + شعار الشركة)
...
| طباعة الشركة | `company_info` | **`PrintSettingsPanel`**: بيانات الشركة فقط — **حفظ التصميم** يخزّن شريط الصفحة + الشريط العائم (`selectionPatches` للخلايا **وأجزاء الهيدر/الفوتر**) |
...
- **مستخلص خدمة:** `purchase_transactions.type = service_ipc` + `suppliers.serviceKind` (labour/equipment/vehicles/housing تحت 21102). بنود: مركز تكلفة + فصل BOQ اختياري + فئة + سابق/حالي/صافي. قيد الاعتماد يوزّع المدين على العقود (`51102` عمال · `51104` معدات/سيارات · `51103` سكن) دون `boq_actual_costs`. **رقم:** تسلسلي قصير `1` · `2` · `6`… (`serviceIpcNumber.ts` — لا UUID). **طباعة:** عنوان `مستخلص — {المورد} {الرقم} (معتمد)` بلا كلمة «خدمة» · شعار الشركة بمسار مطلق في معاينة Blob · الحالة في العنوان. **تنبيه:** `service_ipc_pending` عند التقديم · جرس + deep-link لتبويب `service_ipc`. **موازنة نقدية:** فئة التزام `service` لأوراق 21102 المرتبطة بمقاول خدمة + مستخلصات خدمة مقدَّمة غير معتمدة. **نسب صفر:** لا `Number(pct) || default` — النسبة `0` تُحفظ وتُستعاد؛ نسب الضريبة/الاستقطاع في payload؛ الاحتياطي من المبالغ بـ fallback `0`.
