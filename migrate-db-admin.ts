/**
 * Migration Script: Copy all data from old Firestore DB to new DB
 * Uses Firebase Admin SDK to bypass security rules
 * 
 * Prerequisites:
 * 1. Install admin SDK: npm install firebase-admin
 * 2. Get service account key from Firebase Console
 * 
 * Usage: npx tsx migrate-db-admin.ts
 */
import admin from 'firebase-admin';

const OLD_DB_ID = "ai-studio-ed995a7f-1301-474a-bea7-988b7ce5664c";
const NEW_DB_ID = "megypt15061978";
const PROJECT_ID = "gen-lang-client-0599011721";

// All collections to migrate
const COLLECTIONS = [
  'users',
  'projects',
  'contracts',
  'boq_items',
  'actual_costs',
  'collections',
  'suppliers',
  'admin_expenses',
  'chart_of_accounts',
  'transactions',
  'billing',
  'ipc_progress',
  'settings',
  'cost_centers',
  'purchase_transactions',
];

async function migrateCollection(
  collectionName: string,
  oldDb: admin.firestore.Firestore,
  newDb: admin.firestore.Firestore
) {
  console.log(`\n📦 Migrating: ${collectionName}`);

  try {
    const oldCollection = oldDb.collection(collectionName);
    const snapshot = await oldCollection.get();

    if (snapshot.empty) {
      console.log(`   ⏭️  Empty collection, skipping`);
      return { copied: 0, skipped: 0 };
    }

    let copied = 0;
    let skipped = 0;
    const batch = newDb.batch();

    for (const oldDoc of snapshot.docs) {
      const docId = oldDoc.id;
      const data = oldDoc.data();

      try {
        const newDocRef = newDb.collection(collectionName).doc(docId);
        batch.set(newDocRef, data);
        copied++;
        console.log(`   ✅ ${docId}`);
      } catch (err) {
        skipped++;
        console.error(`   ❌ Failed to copy ${docId}:`, err);
      }
    }

    // Commit all writes at once
    if (copied > 0) {
      await batch.commit();
    }

    console.log(`   📊 ${collectionName}: ${copied} copied, ${skipped} skipped`);
    return { copied, skipped };
  } catch (err: any) {
    console.error(`   ❌ Error reading ${collectionName}:`, err.message);
    return { copied: 0, skipped: 0 };
  }
}

async function main() {
  console.log('🚀 Starting Firestore Migration (Admin SDK)...\n');
  console.log(`   Old DB: ${OLD_DB_ID}`);
  console.log(`   New DB: ${NEW_DB_ID}\n`);

  // Initialize Admin SDK for old database (bypasses security rules)
  const oldApp = admin.initializeApp({
    credential: admin.credential.applicationDefault(),
    projectId: PROJECT_ID,
  }, 'old-app');

  const oldDb = oldApp.firestore();
  oldDb.settings({ databaseId: OLD_DB_ID });

  // Initialize Admin SDK for new database
  const newApp = admin.initializeApp({
    projectId: PROJECT_ID,
  }, 'new-app');

  const newDb = newApp.firestore();
  newDb.settings({ databaseId: NEW_DB_ID });

  let totalCopied = 0;
  let totalSkipped = 0;

  for (const coll of COLLECTIONS) {
    const result = await migrateCollection(coll, oldDb, newDb);
    totalCopied += result.copied;
    totalSkipped += result.skipped;
  }

  console.log('\n' + '='.repeat(50));
  console.log(`✅ Migration Complete!`);
  console.log(`   Total copied: ${totalCopied}`);
  console.log(`   Total skipped: ${totalSkipped}`);
  console.log('='.repeat(50));
}

main().catch(err => {
  console.error('\n❌ Migration failed:', err);
  process.exit(1);
});
