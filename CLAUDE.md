...
| `CashBudget.tsx` | — | **local:** Postgres `cash_budget_*` — **موازنة نقدية** من أرصدة اليومية حتى نهاية الفترة: التزامات (21101/21102/21501 + تعويض 12102 إن قلّ عن الحد) مقابل بنوك 12101 + خزينة 12102 + مستخلصات 12201 · جدول الالتزامات = اسم الحساب الفرعي فقط + مركز التكلفة (سطر لكل مركز) · بعد الاعتماد حذف المستبعد + جدول إجمالي لكل مركز · **بدون قيد GL** · `min_balance` على 12102… |
...
الالتزامات = أرصدة **دائنة** على أوراق الموردين `21101…` ومقاولي الباطن `21102…` + رواتب مستحقة `21501…` + تعويض عهدة `12102…` إن قلّ الرصيد عن `min_balance`. الجدول يعرض **اسم الحساب الفرعي فقط** (بدون أب/كود) + **مركز التكلفة** — إن اشترك الرصيد يظهر سطر لكل مركز. قسم النقدية المتاحة مخفي؛ الإجماليات أعلى التقرير فقط. **بعد الاعتماد:** تُحذف البنود المستبعدة تماماً · يُوزَّع رصيد البنوك `12101` + الصناديق (بدون العهد وبدون مستخلصات `12201`) حسب وزن رصيد كل حساب ثم على أسطر مراكز التكلفة · جدول أسفل التقرير لإجمالي التوزيع لكل مركز. لا قيد يومية. حد العهدة: `max(0, min − (GL − تسويات submitted معلّقة))`.
