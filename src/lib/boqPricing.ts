...
export interface BoqRateInputs {
...
export function computeBoqUnitRate(input: BoqRateInputs) {
...
function resolveProfitPctForCostBudget(rateProfitPct: number): number {
...
export function tenderAmountExcludingProfit(item: {
  tenderQty?: unknown;
  rateMaterials?: unknown;
  rateLabour?: unknown;
  rateEquipment?: unknown;
  rateOverheadPct?: unknown;
  rateProfitPct?: unknown;
  unitRateTotal?: unknown;
  tenderAmount?: unknown;
}): number {
  ...
  if (unit !== 0) return (unit / pf) * qty;
  if (qty !== 0 && ta !== 0) return ta / pf;
  return ta;
}
